...
{
    /// <summary>
    /// A modified versino of the HoloToolkit TapToPlace script.
    /// 
    ...
    public class TapToPlace : MonoBehaviour, IInputClickHandler
    {
        ...
        [Tooltip("Auto-rotate transform to face user when being placed")]
        public bool faceUserWhenPlacing;
            ...
            if (IsBeingPlaced)
            {
                ...
                if (Physics.Raycast(headPosition, gazeDirection, out hitInfo, 30.0f, spatialMappingManager.LayerMask))
                {
                    ...
                    if (PlaceParentOnTap)
                    {
                        ...
                        if (faceUserWhenPlacing)
                        {
                            Quaternion q = Camera.main.transform.localRotation;
                            q.x = 0;
                            q.z = 0;
                            ParentGameObjectToPlace.transform.rotation = q;
                        }
                    }
                    else
                    {
